...
from utils import OpenaiEngine, extract_predication, get_engine
import json
import copy
import asyncio
import shutil
...
def auto_eval(args, task_subset, final_predicted_labels, lock, model):
    ...
    output_json_path = os.path.join(args.output_path, f"WebJudge_Online_Mind2Web_eval_{args.model}_score_threshold_{args.score_threshold}_auto_eval_results.json")
    already_ids = []
    ...
    for task_id in task_subset:
        ...
        if not os.path.isdir(traj_dir) or not os.path.isfile(result_json):
            os.makedirs(args.error_path, exist_ok=True)
            print(f"[ERROR] Missing data for {task_id}, moving to {args.error_path}")
            shutil.move(task_dir, os.path.join(args.error_path, task_id))
            continue
        ...
        if mode == "Autonomous_eval":
            ...
            raise ValueError(f"Unknown mode: WebJudge_Online_Mind2Web_eval")

        response = model.generate(messages)[0]
        predicted_label = extract_predication(response, "WebJudge_Online_Mind2Web_eval")
        ...
        with lock:
            with open(os.path.join(args.output_path, f"WebJudge_Online_Mind2Web_eval_{args.model}_score_threshold_{args.score_threshold}_auto_eval_results.json"), "a+") as f_out:
...
def parallel_eval(args, num_workers=1):
    ...
    print(os.listdir(args.trajectories_dir))
    task_dirs = [
    ...
    chunk_size = len(task_dirs)
    task_subsets = [task_dirs[i:i + chunk_size] for i in range(0, len(task_dirs), chunk_size)]

    #Load model
    model = get_engine("gemini", args.api_key, "gemini-2.5-flash", temperature=0, max_new_tokens=4096, rate_limit=60)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--error_path",  type=str, required=True, help="Where to move tasks with missing files")
    ...
    for level in levels:
        ...
        args.trajectories_dir = os.path.join(args.trajectories_dir, level)
        args.error_path = os.path.join(args.error_path, level+ "_error")
        args.output_path = os.path.join(args.output_path, level + "_result")

        if not os.path.exists(args.trajectories_dir):
            print(f"Trajectories directory for {level} does not exist: {args.trajectories_dir}")
            continue
